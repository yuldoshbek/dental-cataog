...
export default function Sidebar({
  ...
  return (
    <aside
      ...
      } transition-all duration-300 bg-white border-r border-gray-200 flex-col hidden sm:flex shrink-0`}
    >
      {/* Лого + кнопка */}
      <div className={`h-14 border-b border-gray-100 flex items-center ${isSidebarOpen ? 'px-4 justify-between' : 'justify-center'}`}>
        {isSidebarOpen && (
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            <span className="font-bold text-blue-800 text-sm truncate">База знаний</span>
          </div>
        )}
        <button
          onClick={() => setIsSidebarOpen(!isSidebarOpen)}
          className="p-1.5 rounded-lg hover:bg-slate-100 text-gray-400 hover:text-gray-600 transition-colors shrink-0"
